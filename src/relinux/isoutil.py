...
class genISOTree(threading.Thread):
    def run(self):
        logger.logI(tn, "Generating ISO Tree")
        # Make the tree
...
class genISOContents(threading.Thread):
    def run(self, configs):
        logger.logI(tn, "Generating ISO Contents")
        # Copy over the files we need
        logger.logI(tn, "Copying over files we need")
        logger.logV(tn, "Copying preseed files to the ISO tree")
        for i in configs[configutils.preseed]:
            logger.logVV(tn, "Copying " + i + " to the ISO tree")
            copyFile(i, isotreel + "preseed")
        if configutils.parseBoolean(configs[configutils.memtest]):
            logger.logV(tn, "Copying memtest to the ISO tree")
            copyFile("/boot/memtest86+.bin", isotreel + "isolinux/memtest")
        logger.logV(tn, "Copying ISOLINUX to the ISO tree")
        ...
        logger.logVV(tn, "Copying isolinux.cfg to the ISO tree")
        ...
        logger.logV("Editing isolinux.cfg")
        ...
        logger.logI(tn, "Generating files")
        logger.logV(tn, "Writing disk definitions")
        ...
        logger.logV(tn, "Generating package manifests")
        logger.logVV(tn, "Generating filesystem.manifest")
        ...
        writer.close()
        logger.logVV(tn, "Generating filesytem.manifest-remove")
        ...
        if configutils.parseBoolean(configs[configutils.enablewubi]) is True:
            ...
            file.close()
        logger.logI(tn, "Making the ISO compatible with a USB burner")
        logger.logVV(tn, "Writing .disk/info")
        file = open(isotreel + ".disk/info", "w")
        file.write(diskname)
        file.close()
        # No idea why this is needed
        logger.logV(tn, "Making symlink pointing to the ISO root dir")
        os.symlink(isotreel + "ubuntu", isotreel)
        logger.logVV(tn, "Writing release notes URL")
        file = open(isotreel + ".disk/release_notes_url", "w")
        file.write(configs[configutils.url] + "\n")
        file.close()
        logger.logVV(tn, "Writing .disk/base_installable")
        fsutil.touch(isotreel + ".disk/base_installable")
        logger.logVV(tn, "Writing CD Type")
...
class genISO(threading.Thread):
    def run(self, configs):
        logger.logI(tn, "Starting generation of the ISO image")
        ...
        logger.logV(tn, "Generating MD5 sums")
        ...
        for x in fsutil.listdir(isotreel, {"recurse": True}):
            ...
            if i.find("isotree") == -1 and i.find("md5sum") == -1:
                logger.logVV(tn, "Writing MD5 sum of " + i)
        ...
        # -o file              Output image
        logger.logI(tn, "Generating the ISO")
        ...
        logger.logV(tn, "Generating MD5 sum for the ISO")
